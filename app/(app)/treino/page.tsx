...
export default async function WorkoutPage() {
  ...
  return <main className="shell py-6 sm:py-8"><p className="eyebrow">Área de treino</p><h1 className="display mt-2 text-4xl font-bold">Treine sem atrito.</h1><p className="mt-3 max-w-xl leading-7 text-[#657168]">Escolha o treino de hoje, ajuste seu plano e acompanhe o que já foi feito.</p><WorkoutPlanner exercises={exercises} plans={plans} activeSession={activeSession ? { id: activeSession.id, name: activeSession.name, startedAt: activeSession.startedAt?.toISOString() ?? null } : null} recentSessions={recentSessions.map((session) => ({ id: session.id, name: session.name, completedAt: session.completedAt?.toISOString() ?? null }))} /></main>;
}
