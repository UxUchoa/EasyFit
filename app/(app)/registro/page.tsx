...
export default async function FoodLogPage({ searchParams }: { searchParams: Promise<{ date?: string; barcode?: string; scanner?: string }> }) {
  ...
  return (
    <main className="shell py-8">
      ...
      <DiaryClient key={date} date={date} today={today} userScope={user.id} meals={meals} initialBarcode={params.barcode?.slice(0, 14)} initialScanner={params.scanner === "1"} />
    </main>
  );
}
