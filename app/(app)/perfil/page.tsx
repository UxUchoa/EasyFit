...
export default async function ProfilePage() {
  ...
  const currentDateKey = calendarDateKey(new Date(), profile?.timezone ?? "America/Sao_Paulo");
  const referenceDate = parseLogicalDate(currentDateKey)!;
  const calculationInput = profile ? { birthDate: profile.birthDate, biologicalSex: profile.biologicalSex === "male" ? "male" as const : "female" as const, heightCm: Number(profile.heightCm), weightKg: Number(profile.currentWeightKg), activityLevel: profile.activityLevel as "sedentary" | "light" | "moderate" | "very_active", referenceDate } : null;
  const estimates = calculationInput ? { age: ageOnDate(calculationInput.birthDate, referenceDate), bmi: calculateBmi(calculationInput.weightKg, calculationInput.heightCm), bmr: Math.round(calculateBmr(calculationInput)), tdee: Math.round(calculateTdee(calculationInput)) } : null;
  ...
  return <main className="shell py-8">
    ...
    {profile && <BodyMeasurements defaultDate={currentDateKey} defaultWeight={Number(profile.currentWeightKg)} measurements={measurements.map((item) => ({ id: item.id, measuredAt: item.measuredAt.toISOString().slice(0, 10), weightKg: Number(item.weightKg), waistCm: item.waistCm === null ? null : Number(item.waistCm), hipCm: item.hipCm === null ? null : Number(item.hipCm), chestCm: item.chestCm === null ? null : Number(item.chestCm), armCm: item.armCm === null ? null : Number(item.armCm), thighCm: item.thighCm === null ? null : Number(item.thighCm) }))} />}
