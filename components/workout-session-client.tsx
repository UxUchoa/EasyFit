...
export function WorkoutSessionClient({
  ...
  return (
    <>
      ...
      {!readOnly && <div data-testid="workout-rest-controls" className="mt-3 grid min-w-0 max-w-full gap-2 rounded-2xl border border-[#dfe5dc] bg-white p-4 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-end"><div className="field"><label htmlFor="manual-rest">Cronômetro manual</label><select id="manual-rest" value={manualRest} onChange={(event) => setManualRest(Number(event.target.value))}>{[30, 45, 60, 75, 90, 120, 180].map((seconds) => <option key={seconds} value={seconds}>{seconds} segundos</option>)}</select></div><button className="button-secondary w-full !min-h-[3.25rem] sm:w-auto" onClick={() => { setRestSeconds(manualRest); setRestPaused(false); setRestFinished(false); }}>Iniciar descanso</button></div>}
