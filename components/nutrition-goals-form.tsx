...
export function NutritionGoalsForm({ initial }: { initial: Initial }) {
  ...
  const balanceFromMaintenance = initial.goal.calorieTarget - initial.estimates.tdee;
  const absoluteBalance = Math.abs(balanceFromMaintenance);
  const balanceLabel = balanceFromMaintenance < 0
    ? `Déficit estimado de ${absoluteBalance} kcal/dia em relação à manutenção.`
    : balanceFromMaintenance > 0
      ? `Superávit estimado de ${absoluteBalance} kcal/dia em relação à manutenção.`
      : "Meta equivalente à manutenção estimada.";
  ...
  return <section className="card mt-5 p-6 sm:p-7">
    ...
    <div className="mt-5 grid gap-3 sm:grid-cols-4"><div className="rounded-2xl bg-[#f4f6f1] p-4"><b>{initial.estimates.bmi.toFixed(1)}</b><br/><span className="text-xs text-[#657168]">IMC atual</span></div><div className="rounded-2xl bg-[#f4f6f1] p-4"><b>{initial.estimates.bmr} kcal</b><br/><span className="text-xs text-[#657168]">Energia em repouso (TMB)</span></div><div className="rounded-2xl bg-[#f4f6f1] p-4"><b>{initial.estimates.tdee} kcal</b><br/><span className="text-xs text-[#657168]">Manutenção estimada (TDEE)</span></div><div className="rounded-2xl bg-[#f4f6f1] p-4"><b>{initial.goal.calorieTarget} kcal</b><br/><span className="text-xs text-[#657168]">Meta diária vigente</span></div></div>
    <div className="mt-4 rounded-2xl border border-[#d9e5d8] bg-[#f4f8f2] p-4 text-sm leading-6 text-[#405348]"><strong>{balanceLabel}</strong><p className="mt-1">A TMB é a energia estimada para manter as funções vitais em repouso; ela não é a meta de manutenção do peso. Por isso, uma meta de perda pode ficar acima da TMB e ainda permanecer abaixo do gasto diário total.</p></div>
    <details className="mt-4 rounded-2xl border border-[#dfe5dc] p-4 text-sm leading-6"><summary className="cursor-pointer font-black">Como o cálculo funciona</summary><div className="mt-3 text-[#657168]"><p>A TMB é estimada pela fórmula de Mifflin–St Jeor: 10 × peso (kg) + 6,25 × altura (cm) − 5 × idade ({initial.estimates.age}) + constante por sexo de cálculo.</p><p className="mt-2">A manutenção estimada (TDEE) multiplica a TMB pelo fator de atividade: 1,2 sedentário; 1,375 leve; 1,55 moderado; 1,725 muito ativo. É sobre o TDEE — e não sobre a TMB — que a meta usa 85% para redução, 100% para manutenção ou 110% para aumento.</p><p className="mt-2">Macros automáticos: proteína 1,6 g/kg, gordura 0,8 g/kg e o restante das calorias em carboidratos. São estimativas, não diagnóstico ou prescrição médica.</p></div></details>
