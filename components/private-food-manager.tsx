...
export function PrivateFoodManager({ foods }: { foods: PrivateFood[] }) {
  ...
  return <>
    ...
    {foods.length === 0 ? <section className="card mt-3 p-7"><h2 className="text-xl font-black">Nenhum alimento privado</h2><p className="mt-2 text-sm leading-6 text-[#657168]">Cadastre o primeiro pela tela Dieta. Ele aparecerá aqui para manutenção.</p></section> : <div className="mt-3 grid gap-5">{foods.map((food) => <form key={food.id} className="card grid gap-4 p-6 sm:p-7" onSubmit={(event) => update(event, food.id)}>
