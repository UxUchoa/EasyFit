...
function compactSourceLabel(source: string) {
  ...
  if (source === "OPEN_FOOD_FACTS") return "Open Food Facts";
  if (source === "TACO_BR") return "TACO";
  if (source === "PRIVATE") return "Pessoal";
  return source;
}
...
export function ActiveDietPlan({ planName, dayLabel, date, meals }: { planName: string; dayLabel: string; date: string; meals: PlannedMeal[] }) {
  ...
  return <section id="plano-ativo" className="card mt-6 p-4 sm:p-7" aria-labelledby="active-diet-title">
    ...
    <div className="mt-4 grid grid-cols-[minmax(0,1fr)_auto] items-stretch gap-2 sm:gap-3">
      <div className="min-w-0 rounded-2xl bg-[#153d28] px-4 py-3 text-left text-white"><p className="text-[10px] font-black tracking-[.12em] text-white/60">PREVISÃO DO DIA</p><strong className="mt-1 block text-xl leading-none sm:text-2xl">{format(totals.calories)} kcal</strong><p className="mt-2 text-[11px] leading-4 text-white/70">P {format(totals.protein, 1)} g · C {format(totals.carbs, 1)} g · G {format(totals.fat, 1)} g</p></div>
      <button type="button" className="inline-flex min-w-[6.5rem] flex-col items-center justify-center rounded-2xl border border-[#cfd8ce] bg-white px-3 py-2 text-xs font-black text-[#166534] shadow-sm hover:bg-[#edf4eb]" onClick={() => setEditMode((current) => !current)}><span aria-hidden="true" className="text-lg leading-none">{editMode ? "✓" : "✎"}</span><span className="mt-1">{editMode ? "Concluir" : "Editar dieta"}</span></button>
    </div>
    ...
    <div className="mt-5 grid gap-4 lg:grid-cols-2">{currentMeals.map((meal) => {
      ...
      return <article key={meal.label} className="rounded-2xl border border-[#dfe5dc] bg-white p-3.5 sm:p-5">
        <div className="flex items-center justify-between gap-3 border-b border-[#edf0eb] pb-3"><div><h3 className="text-lg font-black leading-tight">{meal.label}</h3>{completed && <span className="mt-1 inline-flex rounded-full bg-[#e9f5e9] px-2 py-1 text-[10px] font-black text-[#166534]">REGISTRADA</span>}</div><div className="text-right"><strong className="text-lg font-black text-[#153d28]">{format(mealCalories)}</strong><p className="text-[10px] font-bold uppercase tracking-wide text-[#657168]">kcal previstas</p></div></div>
        <ul className="mt-3 grid gap-2">{meal.items.map((item) => <li key={item.sourcePointer} className="rounded-xl bg-[#f6f8f4] p-3 text-sm">
          <div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><strong className="leading-5">{item.name}</strong>{item.calories === null && <span className="rounded-full bg-[#fff0b8] px-2 py-1 text-[9px] font-black text-[#725d00]">REVISAR</span>}</div><p className="mt-1 text-xs leading-4 text-[#657168]">{format(item.quantity, 2)} {item.unit}{item.source ? <span title={sourceLabel(item.source)}> · {compactSourceLabel(item.source)}</span> : ""}</p></div><strong className={`shrink-0 rounded-lg px-2 py-1 text-sm ${item.calories === null ? "bg-[#fff0b8] text-[#725d00]" : "bg-white text-[#153d28]"}`}>{item.calories === null ? "— kcal" : `${format(item.calories)} kcal`}</strong></div>
          {item.calories !== null ? <div className="mt-2 flex flex-wrap items-center justify-between gap-x-3 gap-y-2 border-t border-[#e3e9e1] pt-2"><p className="text-[11px] font-bold text-[#657168]">P {item.proteinGrams === null ? "—" : format(item.proteinGrams, 1)} g <span className="mx-1 text-[#c5cec3]">·</span> C {item.carbohydrateGrams === null ? "—" : format(item.carbohydrateGrams, 1)} g <span className="mx-1 text-[#c5cec3]">·</span> G {item.fatGrams === null ? "—" : format(item.fatGrams, 1)} g</p><button type="button" aria-label={`Trocar alimento ${item.name}`} className="min-h-8 rounded-lg px-2 text-xs font-black text-[#166534] underline decoration-[#9fbea5] underline-offset-4 hover:bg-white" onClick={() => openReview(item)}>Trocar</button></div> : <button type="button" aria-label={`Revisar e escolher alimento para ${item.name}`} className="mt-3 flex min-h-11 w-full items-center justify-center rounded-xl border border-[#d8c56e] bg-[#fff8d9] px-3 py-2 text-xs font-black text-[#725d00] shadow-sm hover:bg-[#fff0b8]" onClick={() => openReview(item)}><span aria-hidden="true" className="mr-2 grid size-5 place-items-center rounded-full bg-[#725d00] text-white">!</span>Revisar e escolher alimento</button>}
          {editMode && <form className="mt-3 grid grid-cols-2 gap-2 border-t border-[#e3e9e1] pt-3 sm:grid-cols-[minmax(0,1fr)_7rem_7rem]" onSubmit={(event) => void updatePlanItem(event, item)}>
            <div className="field col-span-2 sm:col-span-1"><label htmlFor={`plan-name-${item.sourcePointer}`}>Nome no plano</label><input id={`plan-name-${item.sourcePointer}`} name="name" defaultValue={item.name} minLength={1} maxLength={180} required /></div>
            ...
            <div className="col-span-2 flex gap-2 sm:col-span-3"><button className="button-secondary !min-h-10 flex-1 !px-3" disabled={itemPending !== null}>{itemPending === item.sourcePointer ? "Salvando…" : "Salvar"}</button><button type="button" className="rounded-full border border-[#f0d5d2] px-4 py-2 text-xs font-black text-[#b42318]" disabled={itemPending !== null} onClick={() => void removePlanItem(item)}>Remover</button></div>
          </form>}
        </li>)}</ul>
