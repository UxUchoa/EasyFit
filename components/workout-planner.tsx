...
type WorkoutPlan = {
  ...
  division: string;
  active: boolean;
  versions: Array<{ version: number; generatedByRuleVersion: string | null; exercises: PlanExercise[] }>;
};
...
export function WorkoutPlanner({
  ...
  const [generation, setGeneration] = useState<{ ruleVersion: string; warnings: string[]; dayLabels: string[] } | null>(null);
  ...
  async function submitPlan(event: FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch(editingId ? `/api/workout-plans/${editingId}` : "/api/workout-plans", {
      ...
      body: JSON.stringify({ name: planName, division, generationRuleVersion: generation?.ruleVersion ?? null, exercises: exercisesPayload }),
    }).catch(() => null);
    if (!response?.ok) {
  ...
  async function applyTemplate() {
    setPending(true);
    setError("");
    const response = await fetch("/api/workout-plans/templates/starter", { method: "POST" }).catch(() => null);
    if (!response?.ok) setError("Não foi possível aplicar o template.");
    else router.refresh();
    setPending(false);
  }
  ...
  async function generatePlan() {
    ...
    const response = await fetch('/api/workout-plans/generate', { method: 'POST' }).catch(() => null);
    const result = response ? await response.json().catch(() => ({})) as { error?: string; proposal?: { ruleVersion: string; name: string; division: string; warnings: string[]; dayLabels: string[]; exercises: Array<{ id: string; name: string; dayIndex: number; targetSets: number; targetReps: string; restSeconds: number }> } } : null;
    if (!response?.ok || !result?.proposal) {
      ...
      setGeneration({ ruleVersion: result.proposal.ruleVersion, warnings: result.proposal.warnings, dayLabels: result.proposal.dayLabels });
      setImportReview(null);
      setExerciseQuery("");
      setShowBuilder(true);
      window.scrollTo({ top: 0, behavior: 'smooth' });
  ...
  return (
    <>
      ...
      <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <button className="button-secondary" disabled={pending} onClick={generatePlan}>Gerar sugestão revisável</button>
        <button className="button-secondary" disabled={pending} onClick={applyTemplate}>Usar template Full body</button>
  ...
  "days": [{
    ...
    "exercises": [{
      ...
      <p className="mt-3 text-xs leading-5 text-[#657168]">O template é uma orientação geral e precisa ser revisado por você. Restrições, dor ou condições de saúde exigem avaliação profissional.</p>
      ...
      {showBuilder && <div className="field mt-2 max-w-xs"><label htmlFor="plan-division">Divisão do plano</label><select id="plan-division" value={division} onChange={(event) => setDivision(event.target.value)}><option value="FULL_BODY">Full body</option><option value="A">A</option><option value="AB">AB</option><option value="ABC">ABC</option><option value="ABCD">ABCD</option><option value="ABCDE">ABCDE</option><option value="CUSTOM">Personalizada</option></select></div>}
      {showBuilder && generation && <section className='mt-4 rounded-2xl border border-[#d9e5b5] bg-[#f8fce9] p-5' aria-labelledby='generation-review-title'><p className='eyebrow'>Regra {generation.ruleVersion}</p><h2 id='generation-review-title' className='mt-2 text-xl font-black'>Revisão obrigatória antes de ativar</h2><div className='mt-3 flex flex-wrap gap-2'>{generation.dayLabels.map((label, index) => <span key={`${label}-${index}`} className='rounded-full bg-white px-3 py-2 text-xs font-black text-[#166534]'>{String.fromCharCode(65 + index)} · {label}</span>)}</div><ul className='mt-3 list-disc space-y-2 pl-5 text-sm leading-6 text-[#657168]'>{generation.warnings.map((warning) => <li key={warning}>{warning}</li>)}</ul><p className='mt-3 text-sm font-bold text-[#166534]'>Altere nome, dias, exercícios, séries, repetições ou descanso abaixo. O plano só será ativado ao pressionar “Salvar plano”.</p></section>}

      {showBuilder && (
        <section className="card mt-4 p-5 sm:p-7" aria-labelledby="builder-title">
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="eyebrow">{importReview ? "Importação JSON" : editingId ? "Nova versão" : "Plano manual"}</p>
