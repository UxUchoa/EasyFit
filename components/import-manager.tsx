...
type ImportItem = { id: string; position: number; dayLabel: string; mealLabel: string; extractedName: string; extractedQuantity: string | null; extractedUnit: string | null; sourcePointer: string; confidence: number; matchedFoodName: string | null; matchedFoodSource: string | null; matchConfidence: number | null; decision: Decision; reviewedName: string | null; reviewedQuantity: string | null; reviewedUnit: string | null; estimatedCalories: number | null; estimatedProteinGrams: number | null; estimatedCarbohydrateGrams: number | null; estimatedFatGrams: number | null };
...
export function ImportManager({ jobs }: { jobs: ImportJob[] }) {
  ...
  return <div className='mt-8 space-y-6'>
    <section className='card p-6 sm:p-7'>
      ...
      <p className='mt-2 text-sm leading-6 text-[#657168]'>O EasyFit procura cada alimento único no catálogo local, USDA FoodData Central e Open Food Facts, reaproveita a correspondência na semana inteira e calcula os nutrientes pela quantidade informada.</p>
    ...
    <section>
      ...
      <div className='mt-4 space-y-5'>{jobs.map((job) => {
        ...
        const automaticItems = job.items.filter((item) => item.decision === 'KEEP');
        const estimatedCalories = automaticItems.reduce((sum, item) => sum + (item.estimatedCalories ?? 0), 0);
        const automaticFoodGroups = [...new Map(automaticItems.map((item) => [item.extractedName, automaticItems.filter((candidate) => candidate.extractedName === item.extractedName).length])).entries()];
        return <article key={job.id} className='card p-6'>
          ...
          {job.status === 'REVIEW' && <>
            <div className={`mt-4 rounded-2xl border p-4 text-sm leading-6 ${pendingItems ? 'border-[#eadc9c] bg-[#fffbed]' : 'border-[#b9d8ba] bg-[#f2f8f1]'}`}><strong>{automaticItems.length} de {job.items.length} itens foram preparados automaticamente.</strong><br />{pendingItems ? `${pendingItems} item(ns) ainda exigem decisão.` : 'Nenhuma revisão manual é necessária.'} Ignorar removerá {ignored} item(ns) do plano final.</div>
            <div className='mt-4 grid gap-3 sm:grid-cols-3'><div className='rounded-2xl bg-[#f4f6f1] p-4'><strong className='text-xl'>{automaticItems.length}</strong><p className='text-xs text-[#657168]'>itens com correspondência</p></div><div className='rounded-2xl bg-[#f4f6f1] p-4'><strong className='text-xl'>{automaticFoodGroups.length}</strong><p className='text-xs text-[#657168]'>alimentos únicos resolvidos</p></div><div className='rounded-2xl bg-[#f4f6f1] p-4'><strong className='text-xl'>{Math.round(estimatedCalories).toLocaleString('pt-BR')} kcal</strong><p className='text-xs text-[#657168]'>estimadas na semana resolvida</p></div></div>
            {automaticItems.length > 0 && <details className='mt-4 rounded-2xl border border-[#dfe5dc] p-4'><summary className='cursor-pointer font-black'>Ver correspondências automáticas ({automaticFoodGroups.length})</summary><ul className='mt-3 grid gap-2 text-sm sm:grid-cols-2'>{automaticFoodGroups.map(([name, count]) => { const sample = automaticItems.find((item) => item.extractedName === name)!; return <li key={name} className='rounded-xl bg-[#f4f6f1] p-3'><strong>{name}</strong><p className='mt-1 text-xs text-[#657168]'>{sample.matchedFoodName} · {sample.matchedFoodSource} · {Math.round((sample.matchConfidence ?? 0) * 100)}% · {count} ocorrência(s)</p></li>; })}</ul></details>}
            {pendingItems > 0 && <div className='mt-5'><h4 className='font-black'>Somente itens que precisam de você</h4><div className='mt-3 space-y-3'>{job.items.filter((item) => item.decision === 'PENDING').map((item) => <form key={item.id} onSubmit={(event) => review(event, job.id, item.id)} className='rounded-2xl border border-[#dfe5dc] p-4'>
